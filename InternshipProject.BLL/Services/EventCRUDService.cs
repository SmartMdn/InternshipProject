...
internal class EventCRUDService : ICRUDService<EventDTO>
{
    ...
    public EventDTO Get(int? id)
    {
        ...
        Event _event = _database.Events.Get(id);
        return _event == null ? throw new ValidationException("Билет не найден", "") : new EventDTO
        {
            ...
            CagegoryId = _event.CategoryId
        };
    }

    public IEnumerable<EventDTO> GetAll()
    {
        IMapper mapper = new MapperConfiguration(cfg => cfg.CreateMap<Event, EventDTO>()).CreateMapper();
    ...
    public void Put(EventDTO item, int? id)
    {
        ...
        IMapper mapper = new MapperConfiguration(cfg => cfg.CreateMap<EventDTO, Event>()).CreateMapper();
        Event _event = mapper.Map<EventDTO, Event>(item);
        _event.Id = (int)id;
